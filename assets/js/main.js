...
function createRow(date, targetMonth) {
    var row = [];
    for (var i = 0; i < 42; i++) {
        var dateCountObj = gatDateCountObject(date);

        row.push({
            date: date.getDate(),
            dayname:
                dayNameList[dateCountObj.day][dateCountObj.count - 1] +
                " " +
                dateName[dateCountObj.day],
            six: "大安",
...
/**
 * 日付オブジェクトから第何何曜日かを算出する
 * @param {*} date
 */
function gatDateCountObject(date) {
    return {
        day: date.getDay(),
